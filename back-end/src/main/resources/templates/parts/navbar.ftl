<#macro navbar>
 <nav class="teal">
    <div class="nav-wrapper">
        <div class="container">
        <img src="https://image.ibb.co/fTSyWU/Osprey_perched_showing_talons1.png" alt="" width="50" height="70" style="margin-left: 0px; margin-bottom: 20px">
        <a href="#" class="brand-logo left">Osprey Fast Bus</a>
        <ul id="nav-mobile" class="right hide-on-med-and-down" style="margin-right: 250px">
            <li><a href="/main">Купить билет</a></li>
            <li><a href="/information">Информация</a></li>
            <!--   <li><a href=/staff">Персонал</a></li> -->
            <li><a href="/profile">Личный кабинет</a></li>
            <li ><a href="/login">Войти на сайт</a></li>
        </ul>

        </div>
    </div>
 </nav>
</#macro>